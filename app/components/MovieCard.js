                ...
                <div className="absolute bottom-0 left-0 right-0 bg-black/70 backdrop-blur-sm p-2 rounded-b-md opacity-0 group-hover:opacity-100 transition-opacity flex justify-around">
                    {!isWatched ? (
                        ...
                        <button
                            ...
                            disabled={isLoading}
                            className="text-xs px-2 py-1 bg-red-500/20 hover:bg-red-500/30 rounded transition-colors disabled:opacity-50"
                        >
                            {isLoading ? 'Loading...' : 'Remove Watched'}
                        </button>
                    )}
                </div>
            </a>

            <a className="px-2 pb-2 flex w-full flex-col gap-1" href={`https://www.themoviedb.org/movie/${movie.imdbID}`}>
