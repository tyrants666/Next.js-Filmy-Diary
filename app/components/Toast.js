...
const Toast = ({ message, type = 'success', isVisible, onClose, duration = 4000 }) => {
    ...
    return (
        <div 
            className={`
                fixed left-1/2 z-50 max-w-sm sm:max-w-md w-full mx-2 sm:mx-4 px-3 sm:px-6 py-2 sm:py-4 
                bg-white border border-gray-200 rounded-lg sm:rounded-xl shadow-lg
                transform transition-all duration-300 ease-out
                ${isAnimating 
                    ? 'bottom-4 sm:bottom-6 -translate-x-1/2 translate-y-0 opacity-100 scale-100' 
        ...
        >
            <div className="flex items-start gap-2 sm:gap-3">
                ...
                <div className="flex-1 min-w-0">
                    <p className="text-xs sm:text-sm font-medium text-gray-900 leading-snug sm:leading-relaxed">
                        {message}
                    </p>
                </div>
                <button
                    onClick={handleClose}
                    className="flex-shrink-0 ml-1 sm:ml-2 p-0.5 sm:p-1 rounded-full hover:bg-gray-100 transition-colors duration-200"
                >
