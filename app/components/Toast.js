...
const Toast = ({ message, type = 'success', isVisible, onClose, duration = 4000 }) => {
    ...
    return (
        <div 
            className={`
                fixed left-1/2 z-50 max-w-md w-full mx-4 px-6 py-4 
                bg-white border border-gray-200 rounded-xl shadow-lg
                transform transition-all duration-300 ease-out
                ${isAnimating 
                    ? 'bottom-6 -translate-x-1/2 translate-y-0 opacity-100 scale-100' 
        ...
        >
            <div className="flex items-start gap-3">
                ...
                <button
                    onClick={handleClose}
                    className="flex-shrink-0 ml-2 p-1 rounded-full hover:bg-gray-100 transition-colors duration-200"
                >
